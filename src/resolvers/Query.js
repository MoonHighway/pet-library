export default {
  petById: (parent, { id }, { pets }) =>
    pets.findOne({ id }),
  allCustomers: (parent, args, { customers }) =>
    customers.find().toArray(),
  me: (parent, args, { currentCustomer }) =>
    currentCustomer,
  totalCustomers: (parent, args, { customers }) =>
    customers.countDocuments(),
  async totalPets(parent, { status }, { pets, checkouts }) {
    console.log("YOU SURE ABOUT THAT", pets);
    if (status === "AVAILABLE") {
  ...
  async allPets(
    ...
    let categorizedPetsArray = await pets
      .find({ category })
      .toArray();
    let checkoutsArray = await checkouts.find().toArray();
    let checkoutIdsArray = checkoutsArray.map(
      (pet) => pet.petId
    );

    if (category && status === "CHECKEDOUT") {
      let checkedOutCategorizedPets =
        categorizedPetsArray.filter((pet) =>
          checkoutIdsArray.includes(pet.id)
        );
      return checkedOutCategorizedPets;
    } else if (category && status === "AVAILABLE") {
      let availableCategorizedPets =
        categorizedPetsArray.filter(
